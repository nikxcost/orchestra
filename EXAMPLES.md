...
```
Напишите SQL запрос для выборки всех активных пользователей, зарегистрированных за последний месяц

Оптимизируйте следующий SQL запрос: SELECT * FROM orders WHERE user_id IN (SELECT id FROM users)

Создайте схему базы данных для системы управления задачами
```

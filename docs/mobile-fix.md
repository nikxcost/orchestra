# 📱 Исправление доступа с мобильных устройств

## Проблема

Приложение работает на ноутбуке, но не работает на телефоне. Ошибка: "Backend не запущен".

## Причина

Frontend использует `localhost:8000` как API URL, что работает только когда backend запущен локально на том же устройстве. На телефоне `localhost` указывает на сам телефон, а не на сервер.

## Решение

### Локально (уже сделано)

✅ Создан файл `.env` с правильным production API URL:
```env
VITE_API_BASE_URL=http://109.69.22.56/api
```

### На сервере (нужно выполнить)

**1. Подключитесь к серверу:**
```bash
ssh root@109.69.22.56
```

**2. Обновите backend `.env` файл:**
```bash
cd /root/orchestra
nano backend/.env
```

**Добавьте следующие строки:**
```env
OPENROUTER_API_KEY=ваш_существующий_ключ
MODEL_NAME=ваша_существующая_модель

# API Key для аутентификации (обязательно!)
API_KEY=CNRaeNw0q5XIMQJNN_9nC8O3X1l5HUT2sD83MXd68xk

# CORS разрешенные origins
ALLOWED_ORIGINS=http://109.69.22.56,http://localhost:5173,http://localhost:3000,http://127.0.0.1:5173
```

⚠️ **ВАЖНО:** API_KEY должен совпадать с VITE_API_KEY из frontend!

**3. Запустите деплой скрипт:**
```bash
cd /root/orchestra
git pull origin main
./deploy.sh
```

**4. Проверьте что сервисы работают:**
```bash
# Проверка backend
sudo systemctl status orchestra

# Если не запущен
sudo systemctl start orchestra

# Проверка nginx
sudo systemctl status nginx

# Проверка логов
sudo journalctl -u orchestra -n 50
```

**5. Проверьте работу API:**
```bash
# Health check
curl http://localhost:8000/health
# Должно вернуть: {"status":"healthy"}

# Через nginx
curl http://109.69.22.56/api/health
# Должно вернуть: {"status":"healthy"}
```

## Проверка с телефона

Откройте в браузере телефона:

1. **Health check:** `http://109.69.22.56/api/health`
   - Должно показать: `{"status":"healthy"}`

2. **Главная страница:** `http://109.69.22.56`
   - Должна загрузиться и работать без ошибок

## Диагностика проблем

### Backend не запускается

```bash
# Посмотреть подробные логи
sudo journalctl -u orchestra -f

# Проверить зависимости
cd /root/orchestra
source venv/bin/activate
cd backend
pip install langchain-openai langchain-core langgraph
```

### CORS ошибки

Проверьте `backend/.env`:
```bash
cat /root/orchestra/backend/.env
```

Убедитесь что `ALLOWED_ORIGINS` включает IP сервера:
```env
ALLOWED_ORIGINS=http://109.69.22.56,http://localhost:5173
```

### Nginx проблемы

```bash
# Проверить конфигурацию
sudo nginx -t

# Посмотреть логи
sudo tail -f /var/log/nginx/orchestra_error.log

# Перезапустить
sudo systemctl restart nginx
```

## Для production (рекомендуется)

1. **Настройте домен** вместо IP адреса
2. **Установите SSL сертификат** (HTTPS)
3. **Настройте API_KEY** для безопасности

См. [DEPLOYMENT.md](../DEPLOYMENT.md) для подробных инструкций.
